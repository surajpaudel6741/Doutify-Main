...
const getUserInfo = asyncHandler(async (req, res) => {
  try {
    ...
    if (user) {
      res.send(user);
      console.log(user);
    } else res.send("user not valid / user not logged in");
  } catch (e) {
...
const putUserInfo = asyncHandler(async (req, res) => {
  try {
    ...
    } else res.status(404).send("user not valid / user not logged in");
  } catch (e) {
...
const getExpertInfo = asyncHandler(async (req, res) => {
  try {
    ...
    if (expert) {
      res.send(expert);
      console.log(expert);
    } else res.send("You have not signed as expert yet");
  } catch (e) {
...
const putExpertInfo = asyncHandler(async (req, res) => {
  try {
    ...
    if (expert) {
      ...
      res.status(202).send("Database is been updated");
    } else res.status(404).send("expert not valid / expert not logged in");
  } catch (e) {
...
// @access private
const getDoubtList = asyncHandler(async (req, res) => {
  // before this, there should be a request to send all doubts done by user
  try {
    const decodedInfo = req.user.decoded;
    const username = decodedInfo.user.username;
    ...
    res
      .status(500)
      .send("An error occurred while fetching user's doubt history");
  }
});
...
// @access private
const putDoubtInfo = asyncHandler(async (req, res) => {
  // before this, there should be a request to send all doubts done by user
  try {
    const decodedInfo = req.user.decoded;
    const username = decodedInfo.user.username;
...
const getExpertMeetings = asyncHandler(async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send("An error occurred while fetching expert's meeting history");
  }
});
...
// @access private
const expertexists = asyncHandler(async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).send("An error occurred while fetching the username");
  }
});
